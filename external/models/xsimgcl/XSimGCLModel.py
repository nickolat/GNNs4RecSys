...
class XSimGCLModel(torch.nn.Module, ABC):
    def __init__(self,
                 num_users,
                 num_items,
                 learning_rate,
    ...
    def cal_cl_loss(self, idx, user_view1, user_view2, item_view1, item_view2):
        ...
        user_cl_loss = self.InfoNCE(user_view1[u_idx], user_view2[u_idx], self.tau)
        item_cl_loss = self.InfoNCE(item_view1[i_idx], item_view2[i_idx], self.tau)
    ...
    def train_step(self, batch):
        gu, gi, cl_user_emb, cl_item_emb = self.propagate_embeddings(perturbed=True)
        user, pos, neg = batch
        user_emb, pos_item_emb, neg_item_emb = gu[user], gi[pos], gi[neg]
        rec_loss = self.bpr_loss(user_emb, pos_item_emb, neg_item_emb)
        cl_loss = self.reg_cl * self.cal_cl_loss([user, pos], gu,cl_user_emb,gi,cl_item_emb)
